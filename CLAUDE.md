...
A 3D "star map" learning page for junior-high study, now **multi-subject** (a subject switcher in the top-left HUD toggles between them; `src/data/subjects.ts` is the registry). Each subject is its own JSON data file rendered into the same 3D scene; the only per-subject behavioral difference is the **answer mode**:

- **物理** (`mode: 'quiz'`) — 15 chapters, 91 knowledge points. Clicking a star opens a knowledge panel → a quiz (2–3 multiple-choice questions); answering all correctly lights the star. Data: `public/data/knowledge-points.json`, sourced from `public/all_knowledge.md` via a Python script.
- **化学** (`mode: 'formula'`) — "元素之谜": 3 chapters (周期表前20位 / 常见金属 / 稀有气体), 31 element stars. Each star is a riddle (谜面); the player **types the element symbol** (`H`, `Na`, `Fe` — the answer is `point.symbol`, NOT the 单质化学式) and the star lights when correct. Matching is **case-sensitive** (chemical symbols require it: `Na` ≠ `na`), whitespace-stripped only. Lit stars reveal symbol + 单质化学式 + explanation instead of asking again. Data: `public/data/chemistry-points.json`, generated by `scripts/gen_chemistry.py` (edit the `ELEMENTS` table there and re-run).

Each knowledge point is a star **randomly scattered across a Ghibli-style daytime sky** (a fixed random spot inside a bounded spherical shell — "漫天星星" — so nothing drifts out of click range); the backdrop is a gradient sky dome + fluffy shader clouds + soft sun glow. Pure frontend, progress persisted in the browser (shared localStorage map keyed by point id; ids don't collide across subjects).
...
1. **Data** (`src/data/`) — fully data-driven, loaded from JSON at runtime.
   ...
   - `DataProvider.tsx` — React Context (`<DataProvider subject={...}>`) that `fetch`es the current subject's JSON (re-fetches when the subject changes) and exposes `useData()` returning `{ subject, mode, chapters, knowledgePoints, chapterColor, loading, error }`. `App.tsx` owns the `subjectId` state and passes the resolved `Subject` in; switching subjects clears the open panel.
   - `subjects.ts` — the subject registry (`SUBJECTS`, `getSubject`, `DEFAULT_SUBJECT`). Each `Subject` has `{ id, name, short, dataUrl, mode }`. Physics `dataUrl` still honors the `VITE_DATA_URL` override.
...
2. **State** (`src/state/useProgress.ts`) — a Zustand store with the `persist` middleware writing to `localStorage` under key `xingtu-progress-v1`.
   - `submitQuiz(id, score, total)` sets `lit = score >= total` (a star lights only when **all** questions are correct) and never un-lits. Returns `{ passed }`. `submitFormula(id, correct)` is the chemistry equivalent — lights the star when `correct` and never un-lits.
...
4. **UI overlay** (`src/ui/`) — plain DOM, absolutely positioned over the canvas. `Hud` (subject switcher + progress + legend; dynamic title/prompt by `useData().mode`), `KnowledgePanel` + `QuizPanel` (physics quiz flow), and `ElementPanel` (chemistry: riddle + formula input, or the symbol/formula/explanation reveal once lit). `App.tsx` branches on `useData().mode` to mount either the quiz panels or `ElementPanel`. `KnowledgeStar` hides the element name (shows "？ 元素之谜") before a formula-mode star is lit, so the title isn't a spoiler. The 3D star's `onClick` → `App.onSelect(id)`; `Canvas onPointerMissed` → closes the panel.
