...
- **化学** (`mode: 'formula'`) — "元素之谜": 3 chapters (周期表前20位 / 常见金属 / 稀有气体), 31 element stars. Each star is a riddle (谜面); the player **types a chemical formula** (element symbol or 单质化学式) and the star lights when correct. Lit stars reveal symbol + formula + explanation instead of asking again. Data: `public/data/chemistry-points.json`, generated by `scripts/gen_chemistry.py` (edit the `ELEMENTS` table there and re-run). Answer matching is forgiving (case-insensitive, subscripts ₂↔2, accepts both symbol and 单质化学式).
